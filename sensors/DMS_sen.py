...
class DMS:
    ...
    def start_reading(self, callback, stop_event):
        ...
        while True:
            self.read_line(self.pin_R1, ["1", "2", "3", "A"], callback)
            self.read_line(self.pin_R2, ["4", "5", "6", "B"], callback)
            self.read_line(self.pin_R3, ["7", "8", "9", "C"], callback)
            self.read_line(self.pin_R4, ["*", "0", "#", "D"], callback)
            time.sleep(0.2)

            if stop_event.is_set():
                break

    def read_line(self, line, characters, callback):
...
def run_dms(dms, callback, stop_event):
    dms.start_reading(callback, stop_event)
