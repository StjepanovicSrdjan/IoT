...
def run_ir_act(callback, settings, stop_event, publish_event, ir_event, mess_queue):
	GPIO.setmode(GPIO.BCM)
	...
	while True:
		...
		for button in range(len(Buttons)):
			if hex(Buttons[button]) == inData:
				...
				callback(publish_event, ButtonsNames[button], settings)
				ir_event.set()
				mess_queue.put(ButtonsNames[button])
